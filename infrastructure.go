...
import (
	...
	"github.com/brsuite/go-socks/socks"
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *bronjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *bronjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled bronjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]bronjson.OutPoint, 0, nslen)
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := bronjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := bronjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
